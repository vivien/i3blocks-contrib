...
void display(int const unit, int const divisor,
             double b, int const warning, int const critical, int const decimals)
{
  char fmtstr[7];
  ...
  int length = decimals + divisor > 1000 ? 5 : 4 + (decimals > 0 ? 1 : 0);
  snprintf(fmtstr, sizeof(fmtstr), "%%%d.%dlf", length, decimals);
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    printf("%s", label);
    display(unit, divisor, rx, warningrx, criticalrx, decimals);
    printf(" ");
    display(unit, divisor, tx, warningtx, criticaltx, decimals);
    printf("\n");
    fflush(stdout);
    s_old = s;
